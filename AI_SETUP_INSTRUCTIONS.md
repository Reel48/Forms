...
## Gemini Live Voice (browser voice)

This repo supports **browser voice chat** using Gemini Live inside the existing backend.

It uses the **same Supabase database** as the chatbot and writes transcript messages into `chat_messages` for the active conversation.
...
**Backend (App Runner)**
- `GEMINI_API_KEY` (AI Studio key; reused)
- `GEMINI_MODEL` (optional)
- `REALTIME_JWT_SECRET` (new: used to mint/verify short-lived voice tokens)
- `VOICE_SYSTEM_PROMPT` (optional)
- `OCHO_USER_ID` (optional)

**Frontend (Vercel)**
- `VITE_API_URL` must point to the backend (already required)

### Local development
Run backend normally, set `VITE_API_URL` to it, and use the microphone button in the customer chat UI.
